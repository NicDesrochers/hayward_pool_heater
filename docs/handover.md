...
- The simulator engine is fixture-derived and native-tested. Playbooks cover `normal_idle`, `config_refresh`, `active_defrost_echo`, `rx_stress`, and `paused`; the CONFIG_5 D06 defrost command/echo behavior is pinned to the active-TX fixture evidence. The simulator emits playbook packets through software-timed open-drain GPIO pulses and listens for controller-originated packets through ESP-IDF 5 RMT RX on the configured half-duplex pin.
- Simulator RX/listen support is intentionally narrow: checksum-valid controller `CONFIG_5` D06 ECO/NORMAL commands emit the fixture-backed heater echo packets, and checksum-valid controller `CONFIG_1` and `CONFIG_2` packets update the simulator's replayed config state. The RX path decodes repeated controller command groups and de-duplicates identical repeats before updating state. Unknown valid controller packets update diagnostics only; invalid checksum/length packets increment simulator errors and do not produce synthetic heater behavior.

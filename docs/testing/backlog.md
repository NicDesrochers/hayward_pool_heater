...
| TST-068 | Simulator RX/listen hardening | ESPHome config/compile / Native C++ unit / Manual HIL | Done | Simulator can listen for controller-originated frames, update known state, and react through fixture-backed echo rules on the shared bus | RMT RX decodes controller packets; heater TX repeats in protocol-shaped groups of four; CONFIG_1 writes update replayed simulator state; CONFIG_5 D06 ECO/NORMAL commands produce delayed fixture-backed heater echoes; broader command behavior remains outside this slice |

...
| TST-067 | First-class simulator component | ESPHome config/compile / Native C++ unit / Analysis tooling | Done | A separate ESP32 simulator node compiles, exposes API entities, and emits fixture-derived playbooks through RMT TX | `examples/hwp-simulator.yaml`; native tests cover wire codec, scheduler, playbooks, and CONFIG_5 D06 echo behavior; CLI tests cover API entity matching |
| TST-068 | Simulator RX/listen hardening | ESPHome config/compile / Native C++ unit / Manual HIL | Done | Simulator can listen for controller-originated frames and react through fixture-backed echo rules on the shared bus | RMT RX decodes controller packets and CONFIG_5 D06 ECO/NORMAL commands produce fixture-backed heater echoes; broader command behavior remains outside this slice |
